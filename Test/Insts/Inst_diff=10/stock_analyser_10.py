...
config_dict=['names','address','codes','FROM','TO','EMAIL','PASSWORD','budget','diff_up','diff_down','alert','inst_diff_up','inst_diff_down','log','path','day_delay']

for i in config_dict:
    exec i + "=Config.get('DEFAULT',i)"

budget, diff_up, diff_down, alert, day_delay, inst_diff_up, inst_diff_down=float(budget),float(diff_up),float(diff_down),float(alert),int(day_delay),float(inst_diff_up),float(inst_diff_down)
    ...
    while valor_venda == 0:
        ...
        if len(valores) < 17:
            ...
            if valor_compra == 0:
                ...
                try: 
                    var_inst = var(variacao, variacao_anterior)
                except:
                    pass
                else:
                    if variacao > 0:
                        ...
                        if var_inst < inst_diff_up:
                            valor_venda = valor_atual
                    else:
                        if variacao < alert:
                            inst_diff_down = diff_down

                            if variacao < diff_down:
                                valor_venda = valor_atual
                        else:
                            inst_diff_down = -1

                        if var_inst < inst_diff_down:
                            valor_venda = valor_atual
